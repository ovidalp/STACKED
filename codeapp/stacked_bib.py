...
def load_DICT_data_from_csv(csv_file, file = False):
    import csv
    try:
            if not file:
...
def get_cmp_system_vs_all(sys_reference,data_dict, save_summary=False,outputdir_csv = "csv/"):
    ...
    if sys_reference in data_dict:
      ...
      dif_cmp = np.array(dif_cmp).tolist()
...
def get_factor_impact(data,rep,alg_name="ANY",show_detail =True):
    ...
    def print_ALLOCATION_table(var_name,var_descrip,var_values,title,imp=1,s_detail =True):
        ...
        if(imp):
            #str_row = "| " + " "*6 + " "*42 + " |"
            message = " * IMPORTANT *"
            str_row = "| " + message+ " "*(48 - len(message))+ " |"
            local_detail+=str_row  + "\n"
            msg = " SS_TV = " + var_name[3]
            for val in var_name[4:]:
                 msg += "+" + val
            str_row = "| " + msg+ " "*(48 - len(msg))+ " |"
            local_detail+=str_row + "\n"
            local_detail+=v_line + "\n"
        if(s_detail):
            print local_detail
...
#---------------------------------------------------------------------------------------------
    ...
    eval_details = "     ############    2^k*r Factorial Designs         #############   \n"
    eval_details += "  Target Algorithm (keypoint detector): " + alg_name + "\n"
    ...
    eval_details += "\n \n--------------------  Prossesing ------------------------------- \n"
    eval_details += "      Observed responses (Y):" + str(m_y) + "\n"
    eval_details += "      Contrasts values (qi): " + str(q) + "\n"
   ...
   # print "  Factors combinations :",factors_combination
    ...
    if show_detail:
          print_ALLOCATION_table(name_variation,descrip_variation,SS,title)
    else:
          eval_details += print_ALLOCATION_table(name_variation,descrip_variation,SS,title,1,0)
    ...
    if show_detail:
          print_ALLOCATION_table(name_variation[3:],SS[3:],SS[3:]/SST*100,title,0)
    else:
          eval_details += print_ALLOCATION_table(name_variation[3:],SS[3:],SS[3:]/SST*100,title,0,0)
    ...
    for i in range(len(fve_MAX)):
       str1 = str('    (%0.2f,%0.2f)'%(fve_min[i],fve_MAX[i]))
